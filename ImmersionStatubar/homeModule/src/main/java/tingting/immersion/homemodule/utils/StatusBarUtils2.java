...
public class StatusBarUtils2 {
    ...
    public static String COLOR_FLAG = "color";
    public static String DRAWABLE_FLAG = "drawable";

    public void init(String bgFlag) {

        fullScreen(mActivity);

        if(COLOR_FLAG.equals(bgFlag)){
            if (mColor != -100) {
                addStatusViewWithColor(mActivity, mColor);//设置了状态栏颜色
            }
        }
        else if(DRAWABLE_FLAG.equals(bgFlag)){
            if (mDrawable != null) {
                addStatusViewWithDrawble(mActivity, mDrawable);//设置了状态栏 drawble，例如渐变色
            }
        }
        else{
    ...
    private void addStatusViewWithColor(Activity activity, int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && color == -1){
                setStatusBarContentBlackColor(activity,true);
            }
        }
    }
